 ...
 * @return      void
 */
class foodbakery_mega_menu_walker extends Walker_Nav_Menu {
	...
	function foodbakery_menu_start() {
		$sub_class = $last = '';
		$count_menu_posts = 0;
		$mega_menu_output = '';
	}
	...
	function start_lvl( &$output, $depth = 0, $args = array(), $id = 0 ) {
		...
		$output .= $this->foodbakery_menu_start();
	...
	function end_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( $this->view === 'main' && $parent_nav_mega == 'on' && $parent_nav_mega_view == 'simple' && $this->child_items_count > 5 && $depth == 1 ) {
			$output .= '<a id="loadMore' . esc_attr( $item_id ) . '" class="btn-view" href="javascript:void(0);">' . esc_html( foodbakery_var_theme_text_srt( 'foodbakery_var_menu_view_more' ) ) . '</a>';
			$output .= '<a id="showLess' . esc_attr( $item_id ) . '" class="btn-less" href="javascript:void(0);">' . esc_html( foodbakery_var_theme_text_srt( 'foodbakery_var_menu_view_less' ) ) . '</a>';
			ob_start();
			?>
			<style type="text/css">
				#showLess<?php _esc_attr( $item_id ); ?>{
					display: none;
				}
				#menulist<?php _esc_attr( $item_id ); ?> li {
					display: none;
				}
			</style>
			<?php
			$foodbakery_inline_script = '   $ = jQuery;
                size_li = $("#menulist<?php _esc_attr($item_id); ?> li").size();
                x = 5;
                $(\'#menulist<?php _esc_attr($item_id); ?> li:lt(\' + x + \')\').show(200);
                $(\'#loadMore<?php _esc_attr($item_id); ?>\').click(function () {
                    size_li = $("#menulist<?php _esc_attr($item_id); ?> li").size();
                    x = (x + 100 <= size_li) ? x + 100 : size_li;
                    $(\'#menulist<?php _esc_attr($item_id); ?> li:lt(\' + x + \')\').show(200);
                    $(\'#showLess<?php _esc_attr($item_id); ?>\').show(0);
                    $(\'#loadMore<?php _esc_attr($item_id); ?>\').hide(0);
                });

                $(\'#showLess<?php _esc_attr($item_id); ?>\').click(function () {
                    x = (x - 100 < 0) ? 5 : x - 100;
                    $(\'#menulist<?php _esc_attr($item_id); ?> li\').not(\':lt(\' + x + \')\').hide(200);
                    $(\'#loadMore<?php _esc_attr($item_id); ?>\').show(0);
                    $(\'#showLess<?php _esc_attr($item_id); ?>\').hide(0);
                });';
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $mega_menu ) && empty( $args->has_children ) && $this->CurrentItem->megamenu == 'on' ) {
			$item_output .= $this->foodbakery_menu_start();
		}
			...
			if ( is_array( $categories ) && sizeof( $categories ) > 0 ) {
				...
				foreach ( $categories as $category ) {
					...
					if ( is_object( $category ) ) {
						...
						if ( $category->count > 4 ) {
							$html .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> <a class="view-all" href="' . esc_url( get_term_link( $category->term_id ) ) . '">' . esc_html__( 'view all', 'foodbakery' ) . ' <i class="icon-angle-double-right"></i></a> </div>';
						}
						...
						foreach ( $posts_array as $menu_post ) {
							...
							if ( $post_thumbnail_id != '' ) {
								$post_thumbnail_url = wp_get_attachment_image_src( $post_thumbnail_id, 'foodbakery_media_4' );
							...
							$post_title = foodbakery_get_post_excerpt( $menu_post->post_title, 6 );
