 ...
 * </code>
 * Example : Note : define your Prefix (like tabs || tab_item ) in pattren. (ref. foodbakery_get_pattern())
 ...
 *  [/tabs]';
 * 	$Output = array();
 *  $PREFIX = 'prefix'; //user prefix as foodbakery_message OR foodbakery_tabs | tab_item
 * 	$parseInstance 	= new ShortcodeParse();
 * 	$output = $b->foodbakery_shortcodes( $output, $text );
 * 	echo '<pre>';
...
if (!class_exists('ShortcodeParse')) {
    ...
    class ShortcodeParse {
        ...
        function foodbakery_get_pattern($content, $PREFIX) {
        ...
        function foodbakery_parse_atts($content) {
        ...
        function foodbakery_shortcodes(&$output, $content, $child = true, $PREFIX) {
            $patts = $this->foodbakery_get_pattern($content, $PREFIX);
            $t = array_filter($this->foodbakery_get_pattern($content, $PREFIX));
            if (!empty($t)) {
                ...
                foreach ($parents as $k => $parent) {
                    ...
                    $t = array_filter($this->foodbakery_get_pattern($contents[$k], $PREFIX));
                    $t_s = $this->foodbakery_shortcodes($outputNew, $contents[$k], true, $PREFIX);
                    $output[$name] = array('name' => $parents[$k]);
                    $output[$name]['atts'] = $this->foodbakery_parse_atts($atts[$k]);
