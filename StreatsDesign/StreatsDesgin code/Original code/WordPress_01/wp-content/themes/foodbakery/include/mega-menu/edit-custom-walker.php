...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( !empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'foodbakery' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)', 'foodbakery' ), $item->title );
		}
				...
				<div class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up" aria-label="<?php esc_attr_e( 'Move up', 'foodbakery' ); ?>">&#8593;</a>
							|
							<a href="<?php
							echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'foodbakery' ); ?>">&#8595;</a>
						</span>
						<a class="item-edit" id="edit-<?php echo absint( $item_id ); ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'foodbakery' ); ?>" href="<?php
						echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'foodbakery' ); ?></a>
					</span>
				</div>
			</div>
			...
			<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo absint( $item_id ); ?>">
				<?php if ( 'custom' == $item->type ) : ?>
					<p class="field-url description description-wide">
						<label for="edit-menu-item-url-<?php echo absint( $item_id ); ?>">
							<?php esc_html_e( 'URL', 'foodbakery' ); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Navigation Label', 'foodbakery' ); ?><br />
				...
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Title Attribute', 'foodbakery' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo absint( $item_id ); ?>">
						...
						<?php esc_html_e( 'Open link in a new tab', 'foodbakery' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)', 'foodbakery' ); ?><br />
				...
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', 'foodbakery' ); ?><br />
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Description', 'foodbakery' ); ?><br />
						...
						<span class="description"><?php esc_html_e( 'The description will be displayed in the menu if the current theme supports it.', 'foodbakery' ); ?></span>
					</label>
				</p>        
				<?php
				...
				<p class="field-custom description description-wide custom_onof">
					<label class="pbwp-checkbox" for="edit-menu-item-megamenu-<?php echo intval( $item_id ); ?>">
						<?php esc_html_e( 'Active Mega Menu', 'foodbakery' ); ?><br />
				...
				<p class="field-view description description-wide">
					<label for="edit-menu-item-view-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Mega Menu View', 'foodbakery' ); ?><br />
						<select id="edit-menu-item-view-<?php echo absint( $item_id ); ?>" onchange="foodbakery_menu_view_select(this.value, '<?php echo absint( $item_id ); ?>')" class="widefat edit-menu-item-view" name="menu-item-view[<?php echo absint( $item_id ); ?>]">
							<option<?php echo esc_attr( $item->view ) == 'simple' ? ' selected="selected"' : '' ?> value="simple"><?php esc_html_e( 'Simple', 'foodbakery' ); ?></option>
							<option<?php echo esc_attr( $item->view ) == 'cat-view-1' ? ' selected="selected"' : '' ?> value="cat-view-1"><?php esc_html_e( 'Categories View 1', 'foodbakery' ); ?></option>
							<option<?php echo esc_attr( $item->view ) == 'cat-view-2' ? ' selected="selected"' : '' ?> value="cat-view-2"><?php esc_html_e( 'Categories View 2', 'foodbakery' ); ?></option>
							<option<?php echo esc_attr( $item->view ) == 'cat-view-3' ? ' selected="selected"' : '' ?> value="cat-view-3"><?php esc_html_e( 'Categories View 3', 'foodbakery' ); ?></option>
						</select>
					</label>
				</p>
				<p id="field-cat-title-<?php echo absint( $item_id ); ?>" class="field-cat-title description description-wide" style="display: <?php echo ($item->view == 'cat-view-2' ? 'block' : 'none') ?>;">
					<label for="edit-menu-item-cat-title-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Title', 'foodbakery' ); ?><br />
				...
				<p id="field-item-categories-<?php echo absint( $item_id ); ?>" class="field-categories description description-wide" style="display: <?php echo ($item->view != 'simple' ? 'block' : 'none') ?>;">
					<label for="edit-menu-item-categories-<?php echo absint( $item_id ); ?>">
						<?php esc_html_e( 'Categories', 'foodbakery' ); ?><br />
						<select id="edit-menu-item-categories-<?php echo absint( $item_id ); ?>" multiple class="widefat edit-menu-item-categories" name="menu-item-categories[<?php echo absint( $item_id ); ?>][]">
							<option value=""><?php esc_html_e( '-- Select Categories --', 'foodbakery' ); ?></option>
							<?php
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__( 'Original: %s', 'foodbakery' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					?>"><?php esc_html_e( 'Remove', 'foodbakery' ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo absint( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
					?>#menu-item-settings-<?php echo absint( $item_id ); ?>"><?php esc_html_e( 'Cancel', 'foodbakery' ); ?></a>
				</div>
