 ...
 * @package Foodbakery
 */
if (!class_exists('Foodbakery_Flickr')) {

    /**
      Foodbakery_Flickr class used to implement the Custom flicker gallery widget.
     */
    class Foodbakery_Flickr extends WP_Widget {

	/**
	 * Sets up a new foodbakery flicker widget instance.
	 */
	public function __construct() {
	    global $foodbakery_var_static_text;
	    parent::__construct(
		    'foodbakery_flickr', // Base ID.
		    foodbakery_var_theme_text_srt('foodbakery_var_flickr_gallery'), // Name.
		    array('classname' => 'widget-gallery', 'description' => foodbakery_var_theme_text_srt('foodbakery_var_flickr_description'))
	    );
	}

	/**
	 * Outputs the foodbakery flicker widget settings form.
	 *
	...
	function form($instance) {
	    ...
	    $foodbakery_opt_array = array(
		'name' => foodbakery_var_theme_text_srt('foodbakery_var_flickr_username'),
		'desc' => '',
		'hint_text' => foodbakery_var_theme_text_srt('foodbakery_var_flickr_username_hint'),
		'echo' => true,
		'field_params' => array(
	    ...
	    $foodbakery_opt_array = array(
		'name' => foodbakery_var_theme_text_srt('foodbakery_var_flickr_photos'),
		'desc' => '',
		'hint_text' => '',
		'echo' => true,
		'field_params' => array(
		    ...
		    'cust_id' => '',
		    'cust_name' => foodbakery_allow_special_char($this->get_field_name('no_of_photos')),
		    'return' => true,
		),
	    );
	...
	/**
	 * Handles updating settings for the current foodbakery flicker widget instance.
	 *
	...
	/**
	 * Outputs the content for the current foodbakery flicker widget instance.
	 *
	...
	function widget($args, $instance) {
	    ...
	    $apikey = $foodbakery_var_options['foodbakery_var_flickr_key'];
	    if ('' !== $apikey) {
		// Getting transient.
		$cachetime = 86400;
		...
		if (false === $check_transient || ($apikey !== $db_apikey || $username !== $db_user_name || $no_of_photos !== $db_total_photos)) {
		    ...
		    if (!isset($user_info->errors) && isset($user_info['stat']) && 'ok' === $user_info['stat']) {
			...
			if ('ok' === $content['stat']) {
			    ...
			    echo esc_html(foodbakery_var_theme_text_srt('foodbakery_var_error')) . $content['code'] . ' - ' . $content['message'];
			}
		    } else {
			if (!isset($user_info->errors)) {
			echo esc_html(foodbakery_var_theme_text_srt('foodbakery_var_error')) . $user_info['code'] . ' - ' . $user_info['message'];
			}
		...
		echo esc_html(foodbakery_var_theme_text_srt('foodbakery_var_flickr_api_key'));
	    }
...
add_action('widgets_init', function() {
    return register_widget("foodbakery_flickr");
});
