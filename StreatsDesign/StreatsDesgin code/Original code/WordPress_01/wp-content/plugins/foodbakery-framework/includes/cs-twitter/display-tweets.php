...
function get_auth( $id, $max_tweets ) {
	...
	$consumer_key = $foodbakery_twitter_arg['consumerkey'];
	$consumer_secret = $foodbakery_twitter_arg['consumersecret'];
	$user_token = $foodbakery_twitter_arg['accesstoken'];
	$user_secret = $foodbakery_twitter_arg['accesstokensecret'];
...
function cache_json( $id, $max_tweets, $time ) {
	...
	$cache = $cache_dir . $id . '.json'; // Twitter cache foodbakery.
	$cache_folder = $cache_dir; // Twitter cache foodbakery.
...
function display_tweets( $id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60 ) {
	...
	if ( ! empty( $tweets ) ) {
		...
		foreach ( $tweets as $tweet ) {
			...
			if ( isset( $style ) ) {
				if ( ! empty( $style ) ) {
					$when = ( 'time_since' === $style ) ? '' : esc_html( foodbakery_var_frame_text_srt( 'foodbakery_var_tweets_time_on' ) );
					$twitter .= '<span>' . $when . '&nbsp;';
		...
		$twitter .= '<li>' . esc_html( foodbakery_var_frame_text_srt( 'foodbakery_var_no_tweets_found' ) ) . '</li>';
	} //end if statement.
