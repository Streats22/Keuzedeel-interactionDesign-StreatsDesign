  ...
  if( $key !== '' ) {
    ...
    if( is_array( $cart_items ) && !empty( $cart_items ) ) {
      ...
      if( is_array( $items_in_cart ) ) {
        ...
        if( isset( $items_in_cart['addon_items'] ) && !empty( $items_in_cart['addon_items'] ) ) {
 ...
 */
function get_formatted_categoriesList( $terms, $cart_key = '' ) {
...
function rp_current_date( $format = '' ) {
  ...
  $current_date = current_time( $date_format );
  return apply_filters( 'rpress_current_date', $current_date );
}
