...
function order_preview_get_order_details( $payment ) {
  ...
  if ( !empty( $payment->customer_id ) ) {
    ...
    if ( empty( $phone ) ) {
      $phone = !empty( $delivery_address_meta['phone'] ) ? $delivery_address_meta['phone'] : 'N/A';
    }

    $flat = !empty( $payment_meta['flat'] ) ? $payment_meta['flat'] : '';

    if ( empty( $flat ) ) {
      $flat = !empty( $delivery_address_meta['flat'] ) ? $delivery_address_meta['flat'] : 'N/A';
    }

    $landmark = !empty( $payment_meta['landmark'] ) ? $payment_meta['landmark'] : '';

    if( empty( $landmark ) ) {
      $landmark = !empty( $delivery_address_meta['landmark'] ) ? $delivery_address_meta['landmark'] : 'N/A';
    }
    ...
    if( empty( $customer_address ) ) {
      $customer_address = !empty( $delivery_address_meta['address'] ) ? $delivery_address_meta['address'] : 'N/A';
    }
  ...
  $service_type = rpress_get_delivery_type( $payment->ID );
  $service_type = !empty( $service_type ) ? $service_type : null;
  ...
  return apply_filters(
    ...
    array(
      ...
      'service_type_slug'         => strtolower( $service_type ),
                ...
                foreach( $fooditems['item_number']['options'] as $addon_items ) :
                  ...
                  ?>
                    <tr class="rp-order-addons">
                      ...
                      <td>
                        <?php 
                         echo rpress_currency_filter( rpress_format_amount( $addon_price ), rpress_get_payment_currency_code( $payment->ID ) ) . ' X ' . $addon_quantity; ?>
                      </td>
  ...
  function get_order_preview_actions_html( $payment ) {
    ...
    if ( $payment_status == 'pending' ) {
      $status_actions['processing'] = array(
        ...
        'payment_id'  => $payment->ID,
        'action'      => 'processing',
        'url'         => wp_nonce_url( admin_url( 'admin-ajax.php?action=rpress_update_order_status&status=processing&current_status='.$current_status.'&payment_id=' . $payment->ID ), 'rpress-mark-order-status' ),
      );
    }


    if ( ( $payment_status == 'processing' || $payment_status == 'pending' ) && $payment_status !== 'completed'  ) {
  ...
  */
              ...
              <div class="rp-order-preview-wrapper">
                <div class="rp-order-preview">
                  ...
                  <div class="rp-order-preview-address">
                    <h2><?php esc_html_e( 'Delivery address', 'restropress' ); ?></h2>
                  
                    <# if ( data.customer_details ) { #>
                      <strong><?php esc_html_e( 'Flat', 'restropress' ); ?> </strong> : 
                      {{ data.customer_details.flat }}
                      <br/>
                      <strong><?php esc_html_e( 'Landmark', 'restropress' ); ?> </strong> : 
                      {{ data.customer_details.landmark }}
                      <br/>
                  ...
                  <div class="order-service-meta">
                    ...
                    <# if ( data.service_time ) { #>
                      <span>
                      <# if ( data.service_type_slug == 'pickup' ) { #>
                        <strong><?php esc_html_e( 'Pickup time', 'restropress' ); ?></strong> :
                      <# } #>

                      <# if ( data.service_type_slug == 'delivery' ) { #>
                        <strong><?php esc_html_e( 'Delivery time', 'restropress' ); ?></strong> : 
                      <# } #>
  ...
  function quick_view_update_order_status() {
    if ( isset( $_GET['status'] ) 
      ...
      wp_safe_redirect( wp_get_referer() ? wp_get_referer() : admin_url( 'edit.php?post_type=fooditem&page=rpress-payment-history' ) );
      exit;
