 ...
 * @return html
*/
function rpress_fooditem_options() {
	...
	if( !empty( $food_item_id ) ) {
		$terms = get_category_by_fooditem( $food_item_id );
		$get_formatted_cats = get_formatted_categories( $terms );
		ob_start();
...
add_action('wp_ajax_rpress_fooditem_options', 'rpress_fooditem_options');
add_action( 'wp_ajax_nopriv_rpress_fooditem_options', 'rpress_fooditem_options');
...
function rpress_ajax_update_cart_items() {
	...
	if( isset( $_POST['fooditem_id'] ) ) {
		...
		$fooditem_id = absint( $_POST['fooditem_id'] );
		$quantity = !empty( $_POST['fooditem_Qty'] ) ? absint( $_POST['fooditem_Qty'] ) : 1;
		$special_instruction = !empty($_POST['special_instruction']) ? $_POST['special_instruction'] : '';

		$options['id'] = $fooditem_id;
		$options['quantity'] = $quantity;
		$options['instruction'] = $special_instruction;

		$item_key = absint( $_POST['fooditem_cartkey'] );
		...
		$return = array(
			...
			'quantity'    => RPRESS()->cart->get_item_quantity( $fooditem_id, $options ),
			'subtotal'    => html_entity_decode( rpress_currency_filter( rpress_format_amount( rpress_get_cart_subtotal() ) ), ENT_COMPAT, 'UTF-8' ),
			'total'       => html_entity_decode( rpress_currency_filter( rpress_format_amount( rpress_get_cart_total() ) ), ENT_COMPAT, 'UTF-8' ),
			'cart_item'   => $items
		);
...
function rpress_ajax_edit_food_item() {	
	...
	$fooditem_id = !empty( $_POST['fooditem_id'] ) ? absint( $_POST['fooditem_id'] ) : '';
	...
	if( !empty( $fooditem_id ) ) {
		...
		$terms = get_category_by_fooditem( $fooditem_id );
		$get_formatted_cats = get_formatted_categories( $terms, $cart_key );
...
function get_formatted_categories( $terms, $cart_key = '' ) {
	if( $terms ) {
		$parent_ids = array();
		$child_ids = array();
    			...
    			if( in_array( $children_data, $child_ids ) ) {
    				...
    				if( is_array($cart_items) ) {
    					...
    					if( in_array( $term_data->name, $cart_items ) ) {
    						$html .= '<input data-type="'.$use_addon_like.'" type='.$use_addon_like.' checked name="'.$name.'" value="'.$term_data->term_id.'|1|'.$term_price.'|'.$use_addon_like.'" id="'.$term_data->slug.'"><span>'.$term_data->name.'</span>';
    					}
    					else {
...
*/
function get_category_by_fooditem($post_id) {
	if( !empty( $post_id ) ) {
...
function rpress_proceed_checkout() {
	...
	$delivery_date = !empty( $_COOKIE['DeliveryDate'] ) ? $_COOKIE['DeliveryDate'] : current_time( 'Y-m-d' );
	...
	$delivery_time = strtotime( $delivery_date . ' ' .$delivery_time );

  //Check minimum order
  $enable_minimum_order = rpress_get_option('allow_minimum_order');

  if ( $enable_minimum_order && $delivery_opt == 'delivery' ) {

    $minimum_order_price = rpress_get_option('minimum_order_price');
    if ( rpress_get_cart_subtotal() < $minimum_order_price ){
  ...
  else if( !empty( $_POST['deliveryTime'] ) && $current_time > $delivery_time ){
  	$time_error = __( 'Error: Please select a different time slot.', 'restropress' );
	$response = array( 
		'status' => 'error',
...
function rpress_get_admin_addon_items() {
  ...
  if( $item_id ) {
    $terms = get_category_by_fooditem($item_id);
