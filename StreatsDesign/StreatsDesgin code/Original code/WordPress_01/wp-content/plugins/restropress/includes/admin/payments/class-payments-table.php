...
<?php
	...
	public function column_default( $payment, $column_name ) {
        ...
        $value   = date_i18n( get_option( 'date_format' ), $date );
			...
			case 'service_date' :
				...
				$service_date_time = strtotime( $service_date_time );
        $value   = date_i18n( get_option( 'date_format' ), $service_date_time ) . ', ' . $service_time ;
				break;
			case 'status' :
        ...
        $status_label = '<mark class="payment-status status-'.$current_status.'" >';
        ...
        $status_label .= '<span> ' . ucfirst( $current_order_status ) . '</span>';
        $status_label .= '</mark>';
        $value = $status_label;
        break;
			default:
    ...
    $order_preview = '<a href="#" class="order-preview" data-order-id="' . absint( $payment->ID ) . '" title="' . esc_attr( __( 'Preview', 'restropress' ) ) . '"><span>' . esc_html( __( 'Preview', 'restropress' ) ) . '</span></a>
      <a class="" href="' . add_query_arg( 'id', $payment->ID, admin_url( 'admin.php?page=rpress-payment-history&view=view-order-details' ) ) . '">#' . $payment->ID . ' ' . $customer_name . '</a><span class="rp-service-type badge-' . $service_type_slug . ' ">' . $service_type . '</span>';
