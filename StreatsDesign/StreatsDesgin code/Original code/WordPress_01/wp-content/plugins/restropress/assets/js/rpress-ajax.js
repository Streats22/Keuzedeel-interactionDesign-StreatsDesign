...
jQuery(document).ready(function($) {
  ...
  $('.rpress-add-to-cart').click(function(e) {
    ...
    if ( typeof Cookies.get('deliveryMethod') == 'undefined' ){
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $('#rpressModal' ).addClass( 'rpress-food-options' ).removeClass('rpress-delivery-options');
        if (item_id !== '' && price !== '') {
  ...
  function rpress_display_service_options( item_id = '' ){
    ...
    $.ajax({
      ...
      success: function( response ) {
        ...
        $( '#rpressModal' ).addClass( 'rpress-delivery-options' ).removeClass('rpress-food-options').modal('show');
        ...
        //Activate the tab
        if ( $('.rpress-tabs-wrapper').length && typeof DeliveryMethod !== 'undefined' ) {
  ...
  $('body').on('click', '.rpress-delivery-opt-update', function(e) {
    ...
    if ( $this.parents('.rpress-tabs-wrapper').find('.nav-item.active a').length > 0 ) {
      var DeliveryMethod = $this.parents('.rpress-tabs-wrapper').find('.nav-item.active a').attr('data-delivery-type');
    }

    var DeliveryTime = $this.parents('.rpress-tabs-wrapper').find('.delivery-settings-wrapper.active .rpress-hrs').val();
    var ServiceDate = $this.parents('.rpress-tabs-wrapper').find('.delivery-settings-wrapper.active .rpress_get_delivery_dates').val();

    if ( ServiceDate == null && $('.rpress_get_delivery_dates').length > 0 ) {
      $this.parents('.rpress-delivery-wrap').find('.rpress-errors-wrap').text('Please select date for ' + DeliveryMethod);
    ...
    if ( DeliveryTime == null && ( rpress_scripts.pickup_time_enabled == 1 && DeliveryMethod == 'pickup'  || rpress_scripts.delivery_time_enabled == 1 && DeliveryMethod == 'delivery' )) {
      $this.parents('.rpress-delivery-wrap').find('.rpress-errors-wrap').text('Please select time for ' + DeliveryMethod);
      $this.parents('.rpress-delivery-wrap').find('.rpress-errors-wrap').removeClass('disabled').addClass('enable');
      return false;
    }
    ...
    Cookies.set('deliveryMethod', DeliveryMethod);

    if ( ServiceDate == null ) {
      Cookies.set( 'DeliveryDate', rpress_scripts.current_date );
    }
    else {
      Cookies.set( 'DeliveryDate', ServiceDate );
    }

    if( DeliveryTime === undefined ) {
      Cookies.set( 'deliveryTime', '' );
    }
    else {
      Cookies.set( 'deliveryTime', DeliveryTime );
    }
      ...
      success: function( response ) {
        if ( response.status == 'error' ) {
          ...
          if ( FoodItemId ) {
            ...
            var DeliveryMethod = Cookies.get('deliveryMethod');
            var DeliveryTime = Cookies.get('deliveryTime');
            var ServiceDate = Cookies.get('DeliveryDate');

            if ( typeof DeliveryMethod !== 'undefined' && typeof DeliveryTime !== 'undefined' ) {
              $('.delivery-items-options').find('.delivery-opts').html('<span class="delMethod">' + DeliveryMethod + ',</span> <span class="delTime"> ' + ServiceDate + ', ' + DeliveryTime + '</span>');
            }
            else if( typeof DeliveryTime == 'undefined' ) {
              $('.delivery-items-options').find('.delivery-opts').html('<span class="delMethod">' + DeliveryMethod + ',</span> <span class="delTime"> ' + ServiceDate + '</span>');
            }
            $('#rpressModal').modal('hide');
          }
  ...
  $('select#rpress-gateway, input.rpress-gateway').change(function(e) {
    ...
    var payment_mode = $('#rpress-gateway, input.rpress-gateway:checked').val();
