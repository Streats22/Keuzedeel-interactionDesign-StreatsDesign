...
class WCFMmp_Frontend {
	...
	public function __construct() {
		...
		add_action( 'wcfm_order_details_after_address', array( &$this, 'wcfmmp_checkout_user_location_show' ), 50 );
		add_action( 'woocommerce_admin_order_data_after_shipping_address', array( &$this, 'wcfmmp_checkout_user_location_show' ), 50 ); 
    ...
    if( $wcfmmp_marketplace_shipping_by_distance_enabled == 'yes' ) $is_allow = true;
	 ...
	 */
	function wcfmmp_checkout_user_location_show( $order ) {
		if( apply_filters( 'wcfmmp_is_allow_checkout_user_location', true ) ) {
			...
			echo '<p><strong>'.__( 'Location', 'wc-multivendor-marketplace' ).':</strong> ' . $address . '</p>';
		}
	}
