	...
	*/
		...
		if($column == 'order_delivery_details')
		{
			...
			if(metadata_exists('post', $post->ID, 'delivery_time') && get_post_meta($post->ID, 'delivery_time', true) !="")
			{
				...
				if($time_format == "H:i" && $minutes[1] == 1440){
    				echo $delivery_time_field_label.": " . date($time_format, mktime(0, $minutes[0])) . ' - ' . "24:00";
    			} else {
    				echo $delivery_time_field_label.": " . date($time_format, mktime(0, $minutes[0])) . ' - ' . date($time_format, mktime(0, $minutes[1]));
    			}
			}
		}
	}
	...
	public function coderockz_woo_delivery_information_after_shipping_address($order){
	    ...
	    if(metadata_exists('post', $order_id, 'delivery_time') && get_post_meta($order_id, 'delivery_time', true) !="") {
	    	...
	    	if($time_format == "H:i" && $minutes[1] == 1440){
				echo '<p><strong>'.$delivery_time_field_label.':</strong> ' . date($time_format, mktime(0, $minutes[0])) . ' - ' . '24:00' . '</p>';
			} else {
				echo '<p><strong>'.$delivery_time_field_label.':</strong> ' . date($time_format, mktime(0, $minutes[0])) . ' - ' . date($time_format, mktime(0, $minutes[1])) . '</p>';
			}
	    }
	...
	public function coderockz_woo_delivery_custom_meta_box() {
		...
		add_meta_box( 'coderockz_woo_delivery_meta_box', __($order_metabox_heading,'coderockz-woo-delivery'), array($this,"coderockz_woo_delivery_meta_box_markup"), 'shop_order', 'side', 'high', null );
	}
	...
	public function coderockz_woo_delivery_meta_box_markup() {
		...
		$delivery_days = isset($delivery_date_settings['delivery_days']) ? $delivery_date_settings['delivery_days'] : "1,2,3,4,5,6,0";
