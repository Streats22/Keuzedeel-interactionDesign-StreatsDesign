...
if (!function_exists('foodbakery_open_close_status')) {
    ...
    function foodbakery_open_close_status($id = '') {
        ...
        if ($restaurant_open === true) {
            ...
            $restaurant_status = esc_html__('Close', 'foodbakery');
        }
                ...
                jQuery(document).ready(function () {
                   ...
                   $(document).on("click", "body", function () {
                    ...
                    $(document).on("click", ".foodbakery-geo-location' . $listing_short_counter . '", function () {
                        ...
                        var data_id = jQuery(this).data("id");
                        jQuery(".foodbakery-locations-field-geo"+data_id).val("");
                        jQuery("#range-hidden-foodbakery-radius' . $listing_short_counter . '").val(' . $default_radius . ');
...
if (!function_exists('restaurant_menu_price_calc')) {
    ...
    function restaurant_menu_price_calc($get_added_menus = '', $foodbakery_restaurant_id = '', $fee = false, $vat = false, $only_vat = false, $converter = false) {
        global $foodbakery_plugin_options;
        ...
        return foodbakery_get_currency($menu_t_price, false, '', '', $converter);
    }
    ...
    for ($menu_loop = 0; $menu_loop < $total_menu_count; $menu_loop ++) {
        ...
        for ($menu_items_loop = 0; $menu_items_loop < $total_items; $menu_items_loop ++) {
            if (isset($restaurant_menu_list[$menu_items_loop]['restaurant_menu']) && $total_menu[$menu_loop] == $restaurant_menu_list[$menu_items_loop]['restaurant_menu']) {
                ...
                <li>
                    ...
                    <div class="price-holder">
                        ...
                        if (isset($menu_item_extra[0]['title']) && is_array($menu_item_extra[0]['title']) && sizeof($menu_item_extra[0]['title']) > 0) {
                            ?>
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#extras-<?php echo absint($menu_loop) ?>-<?php echo absint($menu_items_loop) ?>" data-id="<?php echo absint($menu_items_loop) ?>" data-cid="<?php echo absint($menu_loop) ?>" class="dev-adding-menu-btn"><i class="icon-plus4 text-color"></i></a>
                            <?php
                        } else {
                            ?>
                ...
                if (isset($menu_item_extra[0]['title']) && is_array($menu_item_extra[0]['title']) && sizeof($menu_item_extra[0]['title']) > 0) {
                    ?>
                    <div class="modal fade menu-extras-modal" id="extras-<?php echo absint($menu_loop) ?>-<?php echo absint($menu_items_loop) ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                ...
                                <div class="modal-body">
                                    <div class="menu-selection-container">
                                        ...
                                        foreach ($menu_item_extra['heading'] as $key => $value) {
                                            ...
                                            if (is_array($menu_item_extra_titles) && sizeof($menu_item_extra_titles) > 0) {
                                                ...
                                                <div class="extras-detail-main">
                                                    ...
                                                    <div class="extras-detail-options"<?php echo($menu_extra_counter != 0 ? ' style="display: none;"' : '') ?>>
                                                        <?php
                                                        foreach ($menu_item_extra_titles as $key => $menu_item_extra_title) {
                                                            ...
                                                            <div class="extras-detail-att">
                                                                <label for="extra-<?php echo absint($menu_extra_att_counter) ?>-<?php echo absint($menu_extra_counter) ?>-<?php echo absint($menu_items_loop) ?>">
                                                                    <input type="radio" id="extra-<?php echo absint($menu_extra_att_counter) ?>-<?php echo absint($menu_extra_counter) ?>-<?php echo absint($menu_items_loop) ?>" data-ind="<?php echo absint($menu_extra_att_counter) ?>" data-menucat-id="<?php echo absint($menu_loop) ?>" data-menu-id="<?php echo absint($menu_items_loop) ?>" name="extra-<?php echo absint($menu_extra_counter) ?>-<?php echo absint($menu_items_loop) ?>">
                                        ...
                                        <div class="extras-btns-holder">
                                            <button style="display:none;" data-menucat-id="<?php echo absint($menu_loop) ?>" data-menu-id="<?php echo absint($menu_items_loop) ?>" data-rid="<?php echo absint($foodbakery_restaurant_id) ?>" class="add-extra-menu-btn input-button-loader"><?php esc_html_e('Add to Menu', 'foodbakery') ?></button>
        ...
        if ($menu_it_html != '') {
            ...
            <div class="element-title" id="menu-category-<?php echo absint($menu_loop) ?>">
                ...
                <span><?php echo esc_html($restaurant_menu_cat_desc[$menu_loop]); ?></span>
            </div>
            <ul>
