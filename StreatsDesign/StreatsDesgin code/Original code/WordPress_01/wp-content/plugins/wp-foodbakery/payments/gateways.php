...
if ( isset($_POST['payment_status']) && $_POST['payment_status'] == 'Completed' &&
	...
	if ( isset($booking_id) && ! empty($booking_id) ) {
		$foodbakery_pack_tra_meta = get_post_meta($booking_id, "dir_pakage_transaction_meta", true);
		if ( is_int($foodbakery_pack_tra_meta) ) {
			$foodbakery_pack_tra_meta = array();
		}
		if ( $foodbakery_pack_tra_meta == '' ) {
			$foodbakery_pack_tra_meta = array();
		}
		if ( ! is_array($foodbakery_pack_tra_meta) || empty($foodbakery_pack_tra_meta) || $foodbakery_pack_tra_meta == '' ) {
			$foodbakery_pack_tra_meta = array();
		}
		$trans_counter = 0;
		if ( is_array($foodbakery_pack_tra_meta) && count($foodbakery_pack_tra_meta) > 0 ) {
			$trans_counter = count($foodbakery_pack_tra_meta);
		}
		...
		$index_count = 0;
		$foodbakery_tra_meta = get_option('foodbakery_booking_transaction_meta', true);
		if ( is_int($foodbakery_tra_meta) ) {
			$foodbakery_tra_meta = array();
		}

		if ( ! isset($foodbakery_tra_meta) || empty($foodbakery_tra_meta) || ! is_array($foodbakery_tra_meta) ) {
			$foodbakery_tra_meta = array();
		}

		if ( isset($foodbakery_tra_meta[$booking_id]) && is_array($foodbakery_tra_meta[$booking_id]) && count($foodbakery_tra_meta[$booking_id]) > 0 ) {
			$index_count = (int) count($foodbakery_tra_meta[$booking_id]);
		}
		 ...
		 * All Transactions Data Saved
		  /*---------------------------------------------------- */
		$foodbakery_booking_status = isset($foodbakery_plugin_options['foodbakery_booking_visibility']) ? $foodbakery_plugin_options['foodbakery_booking_visibility'] : 'pending';
		$package_featured_ads = isset($foodbakery_plugin_options['booking_featured_ad_price']) ? $foodbakery_plugin_options['booking_featured_ad_price'] : 0;

		$foodbakery_tra_meta[$booking_id][$index_count][$tnx_type] = $_POST;
		update_option('foodbakery_booking_transaction_meta', $foodbakery_tra_meta);
		$booking_post = array();
		...
		if ( isset($_POST['txn_id']) && $_POST['txn_id'] <> '' ) {
			...
			$foodbakery_pack_tra_meta[$trans_counter] = $transaction_array;

			$payment_date = date_i18n('Y-m-d H:i:s', strtotime(esc_attr($_POST['payment_date'])));

			update_post_meta((int) $booking_id, 'dir_pakage_transaction_meta', $foodbakery_pack_tra_meta);
			...
			$postStatus['post_status'] = $foodbakery_booking_status;
			wp_update_post($postStatus);

			/* ----------------------------------------------------	
			 * Update Featured Status
			  /*---------------------------------------------------- */
			update_post_meta($booking_id, 'foodbakery_booking_pkg_names', $package_id);

			if ( $package_id == '0000000000' ) {
				$package_meta = get_post_meta($booking_id, "_pakage_meta", true);
			} else {
				$foodbakery_packages_options = get_option('foodbakery_packages_options');
				$package_meta = $foodbakery_packages_options[$package_id];
			}

			if ( isset($package_meta['package_duration']) && $package_meta['package_duration'] == 'unlimited' ) {

				update_post_meta((int) $booking_id, 'dir_pkg_expire_date', $foodbakery_current_date);
			...
			/* ----------------------------------------------------	
			 * Update Membership Add Till Date
			  /*---------------------------------------------------- */
			if ( isset($featured) && $featured == 'yes' ) {
				$featured_days = isset($foodbakery_plugin_options['booking_featured_ad_days']) ? $foodbakery_plugin_options['booking_featured_ad_days'] : 0;
