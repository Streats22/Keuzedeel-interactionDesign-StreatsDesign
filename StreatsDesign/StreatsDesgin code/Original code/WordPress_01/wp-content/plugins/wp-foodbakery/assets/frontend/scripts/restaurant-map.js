...
function foodbakery_get_poly_cords_restaurants(db_cords, polygonCoords) {
    ...
    if (typeof polygonCoords !== 'undefined' && polygonCoords != '') {
        ...
        if (typeof db_cords === 'object' && db_cords.length > 0) {
            ...
            jQuery.each(db_cords, function (index, element) {
                ...
                if (resultCord == 'true') {
                    if (resultRestaurants === 0) {
                        list_all_ids += restaurant_id;
                    } else {
                        list_all_ids += ',' + restaurant_id;
                    }
                    resultRestaurants++;
...
function foodbakery_restaurant_map_init(dataobj) {
    ...
    if (latitude != '' && longitude != '') {
        ...
        var map = new google.maps.Map(document.getElementById('foodbakery-restaurant-map-' + map_id), {
            zoom: map_zoom,
            center: mapLatlng,
...
function foodbakery_restaurant_map(dataobj, db_cords) {
    var map_id = dataobj.map_id,
            map_zoom = dataobj.map_zoom,
            this_map_style = dataobj.map_style,
            latitude = dataobj.latitude,
            ...
            plugin_url = foodbakery_loc_strings.plugin_url;
    ...
    if (latitude != '' && longitude != '') {
        ...
        if (typeof polygonCoords !== 'undefined' && polygonCoords != '') {
            ...
            if (typeof db_cords === 'object' && db_cords.length > 0) {
                ...
                if (actual_length > 0 && LatLngList.length > 0) {
                    ...
                    setTimeout(function () {
                        if (mapResizeTimes === 0) {
                            jQuery("#foodbakery-restaurant-map-" + map_id).height(jQuery(window).height);
                    ...
                    jQuery('#restaurant-records-' + map_id).show();
                }
        ...
        function infoContentString(element) {
            ...
            var list_publisher = element.publisher;
            ...
            var contentString = '\
            <div id="restaurant-info-' + restaurant_id + '-' + map_id + '" class="restaurant-info-inner">\
        ...
        function deleteSelectedShape() {
            ...
            var restaurant_form = jQuery('form[id^="frm_restaurant_arg"]');
            ...
            restaurant_form.find('input[name="loc_polygon"]').remove();
        ...
        function updateCurSelText() {
            ...
            var restaurant_form = jQuery('form[id^="frm_restaurant_arg"]');
            var data_vals = restaurant_form.serialize();
            ...
            if (typeof selectedShape.getPath == 'function') {
                ...
                setTimeout(function () {
                    if (resultInsTimes === 0) {
                        var poly_in_restaurants = foodbakery_get_poly_cords_restaurants(db_cords, pathstr);

                        var cords_query_string = jQuery.ajax({
                            url: foodbakery_loc_strings.ajax_url,
                            method: "POST",
                            data: {
                                pathstr: pathstr,
                                poly_in_restaurants: poly_in_restaurants,
                                action: 'foodbakery_restaurants_map_cords_to_url'
                            },
                            dataType: "json"
                            ...
                            setTimeout(function () {
                                if (resultCordTimes === 0) {
                                    ...
                                    if (typeof response.string !== 'undefined' && response.string != '') {
                                        ...
                                        if (jQuery('#foodbakery-restaurant-map-' + map_id).is(':visible')) {
                                            data_vals = '&loc_polygon=' + response.string + '&ajax_filter=true';
                                            var current_url = location.protocol + "//" + location.host + location.pathname + "?" + data_vals;//window.location.href;
                                            window.history.pushState(null, null, decodeURIComponent(current_url));
                                            restaurant_form.find('input[name="loc_polygon"]').remove();
                                            restaurant_form.find('input[name="location"]').val('');
                                            restaurant_form.append('<input type="hidden" name="loc_polygon" value="' + response.string + '">');
                                            //
                                            var restaurant_holder_counter = jQuery('.foodbakery-dev-restaurant-content').data('id');
                                            foodbakery_restaurant_content(restaurant_holder_counter);
                                        }
                                    }
        ...
        google.maps.event.addDomListener(document.getElementById('draw-map-' + map_id), 'click', function () {
            ...
            jQuery('#restaurant-records-' + map_id).hide();
...
jQuery("div[id^='foodbakery-restaurant-map-']").css("pointer-events", "none");
...
var onMapClickHandler = function (event) {
    ...
    that.find('div[id^="foodbakery-restaurant-map-"]').css("pointer-events", "auto");

    // Handle the mouse leave event
    that.on('mouseleave', onMapMouseleaveHandler);
}
jQuery(document).on('click', '.foodbakery-restaurant-map', onMapClickHandler);