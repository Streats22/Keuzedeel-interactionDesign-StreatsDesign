                    ...
                    <div class="list-post <?php echo esc_html($featured_class); ?>">
							...
							<div class="img-holder">
								<figure>
									<a href="<?php the_permalink(); ?>">
										<?php the_post_thumbnail('full', array('class' => 'img-thumb')); ?>
									</a>
								</figure>
							</div>
						<?php } ?>
						<?php if ( $open_close_show_labels == 'yes' && has_post_thumbnail()) : ?>
							<?php $status = foodbakery_open_close_status($restaurant_id); ?>
							<span class="restaurant-status <?php echo strtolower($status); ?>"><em class="bookmarkRibbon"></em><?php echo esc_html__($status, 'foodbakery'); ?></span>
						<?php endif; ?>
                        <?php
                        $ratings_data = array(
                            'overall_rating' => 0.0,
                        ...
                        <div class="text-holder">
                            ...
                            if ($foodbakery_cate_str != '') {
                                ?>
                                <address><?php echo esc_html($foodbakery_cate_str); ?></address>
                                <?php
                            }
                            ?>
                            <div class="delivery-potions">
								<?php if ($restaurant_minimum_order_value != '') { ?>
									<div class="post-time">
                                        <i class="icon-check_circle"></i>
										<?php printf(esc_html__('Min %s'), esc_html(foodbakery_get_currency($restaurant_minimum_order_value, true))); ?>
									</div>
								<?php } ?>
                                <?php
                                if (($restaurant_pickup_delivery == 'delivery' || $restaurant_pickup_delivery) == 'delivery_and_pickup' && $restaurant_delivery_time != '' ) {
                                    ?>
                                    <div class="post-time">
                                        <i class="icon-motorcycle"></i>
										<?php printf(esc_html__('%s min'), esc_html($restaurant_delivery_time)) ?>
                                    </div>
                                    <?php
                                }
								
                                if (($restaurant_pickup_delivery == 'pickup' || $restaurant_pickup_delivery) == 'delivery_and_pickup' && $restaurant_pickup_time != '' ) {
                                ?>
									<div class="post-time">
                                        <i class="icon-clock4"></i>
										<?php printf(esc_html__('%s min'), esc_html($restaurant_pickup_time)) ?>
                                    </div>
                                <?php } ?>
