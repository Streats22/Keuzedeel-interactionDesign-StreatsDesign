...
if ( ! class_exists( 'Foodbakery_contact_form_received_email_template' ) ) {

	class Foodbakery_contact_form_received_email_template {
		...
		public function __construct() {
			...
			add_action( 'foodbakery_contact_form_received_email', array( $this, 'foodbakery_contact_form_received_email_callback' ), 10, 2 );
		}

		public function foodbakery_contact_form_received_email_callback( $user = '', $contact_form = array() ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Reservation Received', 'foodbakery' );
				...
				Foodbakery_contact_form_received_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__( 'This template is used to send email when new contact form received.', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
	...
	new Foodbakery_reservation_received_email_template();
}
