 ...
 * @package	Foodbakery
 */
if ( ! class_exists( 'Foodbakery_approved_publisher_profile_template' ) ) {

	class Foodbakery_approved_publisher_profile_template {
		...
		public function publisher_profile_status_changed( $publisher_id, $publisher_old_status ) {
			...
			if ( $publisher_id != '' ) {
				...
				$publisher = new WP_User( $publisher_id );
				$role = array_shift( $publisher->roles );
				// checking publisher role
				if ( $role == 'foodbakery_publisher' ) {
					...
					// checking job status
					if ( $publisher_status == 'active' && $publisher_status != $publisher_old_status ) {
						...
						if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
							...
							$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__('Approved Publisher Profile', 'foodbakery' );
							$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $blogname ) . ' <' . $admin_email . '>';
							$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_approved_publisher_email();
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__('This template is used to sending email when publisher Status approved', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
	...
	new foodbakery_approved_publisher_profile_template();
}