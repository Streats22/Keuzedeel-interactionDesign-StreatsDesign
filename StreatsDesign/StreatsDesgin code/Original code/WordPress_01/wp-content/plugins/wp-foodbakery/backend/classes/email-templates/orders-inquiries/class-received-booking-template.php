<?php
/**
 * Received Booking Email Template
 *
 * @since 1.0
 * @package	Foodbakery
 */

if ( ! class_exists( 'Foodbakery_received_booking_email_template' ) ) {

	class Foodbakery_received_booking_email_template {
		...
		public function __construct() {
			...
			$this->email_template_index = 'received-booking-template';
			...
			add_action( 'foodbakery_received_booking_email', array( $this, 'foodbakery_received_booking_email_callback' ), 10, 4 );
		}

		public function foodbakery_received_booking_email_callback( $booking_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Received Booking', 'foodbakery' );
				$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $this->get_booking_user_name() ) . ' <' . $this->get_booking_user_email() . '>';
				...
				Foodbakery_received_booking_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__( 'This template is used to send email when restaurant user receive booking.', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
	...
	new Foodbakery_received_booking_email_template();
}
