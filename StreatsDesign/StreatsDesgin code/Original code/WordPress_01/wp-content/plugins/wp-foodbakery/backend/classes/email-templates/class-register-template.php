 ...
 */
if ( ! class_exists( 'Foodbakery_register_email_template' ) ) {

	class Foodbakery_register_email_template {
		...
		public function __construct() {
			...
			add_filter( 'foodbakery_email_template_settings', array( $this, 'template_settings_callback' ), 10, 1 );
			add_action( 'foodbakery_user_register', array( $this, 'publisher_register_callback' ), 10, 2 );
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__( 'This template used to send email when user register.', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
			do_action( 'foodbakery_load_email_templates', $email_templates );
		}

		public function publisher_register_callback( $user = '', $password = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Registerd Successfully', 'foodbakery' );
				...
				do_action( 'foodbakery_send_mail', $email_args );
				foodbakery_register_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}

	}

	return new foodbakery_register_email_template();
}
