 ...
 * @package	Foodbakery
 */
if (!class_exists('Foodbakery_sent_order_email_template')) {

    class Foodbakery_sent_order_email_template {
        ...
        public function __construct() {
            ...
            add_action('foodbakery_sent_order_email', array($this, 'foodbakery_sent_order_email_callback'), 11, 1);
        }

        public function foodbakery_sent_order_email_callback($order_id = '') {
            ...
            if (isset($template['email_notification']) && $template['email_notification'] == 1) {
                ...
                $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : esc_html__('Sent Order', 'foodbakery');
                ...
                Foodbakery_sent_order_email_template::$is_email_sent1 = $this->is_email_sent;
            }
        }
        ...
        public function add_email_template() {
            ...
            $email_templates[$this->template_group][$this->email_template_index] = array(
                ...
                'description' => esc_html__('This template is used to send email when user submit order.', 'foodbakery'),
                'jh_email_type' => 'html',
            );
            ...
            if ($order_type == 'order' && is_array($order_menu_list)) {
                ...
                foreach ($order_menu_list as $_menu_list) {
                    ...
                    if (is_array($extras_item) && sizeof($extras_item) > 0) {
                        ...
                        foreach ($extras_item as $extra_item) {
                            ...
                            if ($title_extra_item != '') {
                                $html .= '<li>' . $heading_extra_item . ' - ' . $title_extra_item . ' : <span class="category-price">' . foodbakery_get_currency($price_extra_item, true, '', '', true) . '</span></li>';
                            }
                ...
                if ($order_m_total > 0) {
                    ...
                    if ($foodbakery_vat_switch == 'on' && $foodbakery_payment_vat > 0) {
                        $html .= '<li>' . sprintf(esc_html__('VAT (%s&#37;)', 'foodbakery'), $foodbakery_payment_vat) . ' <span class="price">' . foodbakery_get_currency_sign() . restaurant_menu_price_calc('defined', $order_m_total, $menu_order_fee, true, true, false) . '</span></li>';
                    }
    ...
    new Foodbakery_sent_order_email_template();
}
