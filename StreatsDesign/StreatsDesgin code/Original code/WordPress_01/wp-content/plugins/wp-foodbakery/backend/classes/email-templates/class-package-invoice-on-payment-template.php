<?php
/**
 * Membership Payment Invoice Email Template.
 *
 * @since 1.0
 * @package	Foodbakery
 */

if ( ! class_exists( 'Foodbakery_package_user_invoice_template' ) ) {

	class Foodbakery_package_user_invoice_template {
		...
		public function __construct() {
			...
			$this->email_template_variables = array(
				...
				array(
					...
					'display_text' => 'Membership Payment Invoice',
			...
			add_action( 'foodbakery_package_invoice_on_payment_email', array( $this, 'foodbakery_package_invoice_on_payment_email_callback' ), 10, 2 );
		}

		public function foodbakery_package_invoice_on_payment_email_callback( $user = '', $package_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Membership Expired', 'foodbakery' );
				...
				Foodbakery_package_user_invoice_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
	...
	new Foodbakery_package_user_invoice_template();
}
