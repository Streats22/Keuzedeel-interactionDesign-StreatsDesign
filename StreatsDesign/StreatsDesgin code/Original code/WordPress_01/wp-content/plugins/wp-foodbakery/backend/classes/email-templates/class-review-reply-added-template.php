...
if ( ! class_exists( 'Foodbakery_review_reply_added_email_template' ) ) {

	class Foodbakery_review_reply_added_email_template {
		...
		public function __construct() {
			...
			add_action( 'foodbakery_review_reply_added_email', array( $this, 'foodbakery_review_reply_added_email_callback' ), 10, 2 );
		}

		public function foodbakery_review_reply_added_email_callback( $user = '', $review_id = '' ) {

			$this->user = $user;
			$this->review_id = $review_id;
                        $reciever_email = '';
			$template = $this->get_template();
                        $review_reply_parent    = get_post_meta( $review_id, 'foodbakery_parent_review', true );
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Review Reply Added', 'foodbakery' );
				...
				Foodbakery_review_reply_added_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__( 'This template is used to send email when reply for a review added.', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
	...
	new Foodbakery_review_reply_added_email_template();
}
