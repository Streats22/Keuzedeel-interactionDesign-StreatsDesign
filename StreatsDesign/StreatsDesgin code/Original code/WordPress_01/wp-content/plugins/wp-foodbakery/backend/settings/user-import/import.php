...
if (!defined('FOODBAKERYV_DELIMITER'))
    define('FOODBAKERYV_DELIMITER', ',');
...
class foodbakery_user_import {
    ...
    public function __construct() {
        //add_action('admin_menu', array(&$this, 'foodbakery_user_import_page'));
        add_action('init', array(&$this, 'foodbakery_import_user_process'));
        add_action('init', array(&$this, 'foodbakery_export_user_process'));
     ...
     * @since 0.1
     * */
    public function foodbakery_user_import_page() {
        add_submenu_page('edit.php?post_type=restaurants', foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_update' ), foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_update' ), 'manage_options', 'user-import', array(&$this, 'foodbakery_import_user_form'));
    }

    /**
     * Import Users from foodbakery_framework Process
     *
     * @since 0.1
     * */
    public function foodbakery_import_user_demodata($password_nag = false, $new_user_notification = false, $users_update = false, $filename = "") {
        ...
        WP_Filesystem();
        $unzipfile = unzip_file($filename, $foodbakery_upload_dir['path'] . '/' . $first_str_filename . '/');
        // delete zip archive
        unlink($filename);

        $withoutExt = preg_replace('/\\.[^.\\s]{3,4}$/', '', $filename);
        $csv_filename = $foodbakery_upload_dir['path'] . '/' . $first_str_filename . '/' . $first_str_filename . '.csv';
        ...
        $images_foodbakery = $foodbakery_upload_dir['path'] . '/' . $first_str_filename;
        if (is_dir($images_foodbakery)) {

            if ($dh = opendir($images_foodbakery)) {

                while (($file = readdir($dh)) !== false) {
                    $image_filename_url = $foodbakery_upload_dir['path'] . '/' . $first_str_filename . '/' . $file;
                    $newfile = $foodbakery_upload_dir['path'] . '/' . $file;
                    if (is_file($image_filename_url)) {
                        //foodbakery_import_user_profile_images($image_filename_url, $file);
                    }
                }
                closedir($dh);
                // move all image
     ...
     * @since 0.1
     * */
    public function foodbakery_import_user_process() {
     ...
     * @since 0.1
     * */
    public function foodbakery_export_user_process() {
        global $wpdb;
        if (isset($_POST['_wpnonce-cs-export-users-page'])) {
            ...
            if (isset($_POST['btn-export-users'])) {
                ...
                $existing_file = $foodbakery_upload_dir['path'] . '/';
                $existing_file_url = $foodbakery_upload_dir['url'] . '/';
                ...
                if ($__user) {
                    ...
                    // create zip file
                    $zip = new ZipArchive;
                    $zip->open($foodbakery_upload_dir['path'] . '/' . $zip_file_name, ZipArchive::CREATE);
                    ...
                    $filepath = $foodbakery_upload_dir['path'] . "/";
                    global $wp_filesystem;
                    ...
                    if ($wp_filesystem->put_contents($filename, $fields)) {
                        ...
                        echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_data_import_error' );
                    }
                }
                // Set everything back to normal.
                remove_filter('upload_dir', 'foodbakery_user_images_custom_foodbakery');
     ...
     * @since 0.1
     * */
    public function foodbakery_import_user_form() {
        if (!current_user_can('create_users'))
            wp_die( foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_sufficient_permissions' ) );
        ?>
        ...
        <div class="wrap">
            ...
            if (!file_exists($error_log_file)) {
                if (!@fopen($error_log_file, 'x'))
                    echo '<div class="updated"><p><strong>' . sprintf( foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_import_notice' ), self::$log_dir_path) . '</strong></p></div>';
            }

            if (isset($_GET['import'])) {
                $error_log_msg = '';
                if (file_exists($error_log_file))
                    $error_log_msg = sprintf(esc_html__(', please <a href="%s">check the error log</a>', 'foodbakery'), $error_log_url);

                switch ($_GET['import']) {
                    case 'file':
                        echo '<div class="error"><p><strong>' . foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_error_file_upload' ) . '</strong></p></div>';
                        break;
                    case 'data':
                        echo '<div class="error"><p><strong>' . foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_cannot_extract_data' ) . '</strong></p></div>';
                        break;
                    case 'fail':
                        echo '<div class="error"><p><strong>' . sprintf( foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_not_imported' ), $error_log_msg) . '</strong></p></div>';
                        break;
                    case 'errors':
                        echo '<div class="error"><p><strong>' . sprintf( foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_imported_some_success' ), $error_log_msg) . '</strong></p></div>';
                        break;
                    case 'success':
                        echo '<div class="updated"><p><strong>' . foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_import_successful' ) . '</strong></p></div>';
                        break;
                    case 'filetype_error':
                        echo '<div class="error"><p><strong>' . foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_invalid_file_type' ) . '</strong></p></div>';
                        break;
                    default:
                        break;
                }
            ...
            if (isset($_GET['export'])) {
                $error_log_msg = '';
                if (file_exists($error_log_file))
                    $error_log_msg = sprintf(__(', please <a href="%s">check the error log</a>', 'foodbakery'), $error_log_url);

                switch ($_GET['export']) {
                    case 'success':
                        echo '<div class="updated"><p><strong>' . foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_export_successful' ) . '</strong></p></div>';
                        break;
                    default:
                        break;
                }
            }//['foodbakery_restaurant_users_zip_file_desc
            ?>
            ...
            <form method="post" action="" enctype="multipart/form-data">
                ...
                <div class="form-elements">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                    	<label for="users_csv"><?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_zip_file' ); ?></label>
                    </div>
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <input type="file" id="users_csv" name="users_csv" value="" class="all-options" /><br />
                        <span class="description"><?php echo sprintf(foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_zip_file_desc' ), plugin_dir_url(__FILE__) . 'demo/user_data.zip'); ?></span>
                    </div>
                </div>
                <div class="form-elements">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<label for="new_user_notification"><?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_zip_notification' ); ?></label>
                    </div>
                	<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <input id="new_user_notification" name="new_user_notification" type="checkbox" value="1" />
                        <?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_zip_send_new_users' ) ?>
                    </div>
                </div>
                <div class="form-elements">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<label for="new_user_notification"><?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_password_nag' ); ?></label>
                    </div>
                	<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <input id="password_nag" name="password_nag" type="checkbox" value="1" />
                        <?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_password_nag_hint' ) ?>
                    </div>
                </div>
                <div class="form-elements">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<label for="new_user_notification"><?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_update' ); ?></label>
                    </div>
                	<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <input id="users_update" name="users_update" type="checkbox" value="1" />
                        <?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_update_hint' ) ?>
                    </div>
                </div>
                ...
                <div class="form-elements">
                    ...
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <p class="submit">
                            <input type="submit" name="btn-import-users" class="button-primary" value="<?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_import_users' ); ?>" />
                        </p>
                    </div>
                </div>
            ...
            <form method="post" action="" enctype="multipart/form-data">
                ...
                <div class="form-elements">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                    	<label><?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_export_all_users' ); ?></label>
                    </div>
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                        <input type="submit" name="btn-export-users" class="button-primary" value="<?php echo foodbakery_plugin_text_srt( 'foodbakery_restaurant_users_export' ); ?>" />
                    </div>
                </div>
            </form>   
        </div>
    ...
    public static function import_csv($filename, $args) {
        ...
        $csv_reader = new foodbakeryv_reader($file_handle, FOODBAKERYV_DELIMITER, "\xEF\xBB\xBF"); // Skip any UTF-8 byte order mark.
    ...
    private static function log_errors($errors) {
        ...
        @fwrite($log, sprintf(esc_html__('BEGIN %s', 'foodbakery'), date('Y-m-d H:i:s', time())) . "\n");

        foreach ($errors as $key => $error) {
            $line = $key + 1;
            $message = $error->get_error_message();
            @fwrite($log, sprintf(esc_html__('[Line %1$s] %2$s', 'foodbakery'), $line, $message) . "\n");
        }
