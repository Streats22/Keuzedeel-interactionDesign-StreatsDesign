...
/**
 * File Type: Buyer Profile Approved Email Template
 */
if (!class_exists('foodbakery_not_approved_buyer_profile_template')) {

    class foodbakery_not_approved_buyer_profile_template {
        ...
        function get_not_approved_buyer_buyer_name() {
            $buyer_info = get_user_by('id', $this->buyer_id);
            return $buyer_info->display_name;
        }

        function get_not_approved_buyer_buyer_email() {
            $buyer_info = get_user_by('id', $this->buyer_id);
        ...
        public function buyer_profile_status_changed($buyer_id, $buyer_old_status) {
            ...
            if ($buyer_id != '') {
                ...
                if ($role == 'foodbakery_buyer') {
                    ...
                    // checking job status
                    if ($buyer_status == 'inactive' && $buyer_status != $buyer_old_status) {
                        ...
                        if (isset($template['email_notification']) && $template['email_notification'] == 1) {
                            ...
                            $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Not Approved Buyer Profile', 'foodbakery');
                            $from = (isset($template['from']) && $template['from'] != '') ? $template['from'] : esc_attr($blogname) . ' <' . $admin_email . '>';
                            $recipients = (isset($template['recipients']) && $template['recipients'] != '') ? $template['recipients'] : $this->get_not_approved_buyer_buyer_email();
        ...
        public function add_email_template() {
            ...
            $email_templates[$this->template_group][$this->email_template_index] = array(
                ...
                'description' => __('This template is used to sending email when buyer Status not approved', 'foodbakery'),
                'jh_email_type' => 'html',
            );
    ...
    new foodbakery_not_approved_buyer_profile_template();
}