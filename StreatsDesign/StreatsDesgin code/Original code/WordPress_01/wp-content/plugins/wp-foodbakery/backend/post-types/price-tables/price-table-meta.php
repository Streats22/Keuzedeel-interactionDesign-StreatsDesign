...
if (!class_exists('price_tables_post_type_meta')) {
	...
	public function __construct() {
	    ...
	    add_action('save_post', array($this, 'foodbakery_insert_price_table_metas'), 19);
	    add_action('wp_ajax_foodbakery_pt_iconpicker', array($this, 'foodbakery_pt_icon'));

	    add_action('admin_menu', array($this, 'remove_cus_meta_boxes'));
	    add_action('do_meta_boxes', array($this, 'remove_cus_meta_boxes'));
	    add_filter("get_user_option_screen_layout_foodbakery-pt", array($this, 'restaurant_type_screen_layout'));
	}
	...
	function remove_cus_meta_boxes() {
	    remove_meta_box('submitdiv', 'foodbakery-pt', 'side');
	}
	...
	public function price_tables_add_meta_boxes_callback() {
	    add_meta_box('foodbakery_meta_price_tables', esc_html(foodbakery_plugin_text_srt('foodbakery_restaurant_price_tables_options')), array($this, 'foodbakery_meta_price_tables'), 'foodbakery-pt', 'normal', 'high');
	}

	/**
	 * Creating an array for meta fields
	 */
	public function foodbakery_meta_price_tables() {
	    global $post, $foodbakery_html_fields;

	    $rand_numb = rand(1000000, 9999999);
	    ?>
	    <div class="foodbakery-price-table">
	        <div class="row">
	    	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<?php
			$foodbakery_opt_array = array(
			    'name' => __('Membership Subscribe Action', 'foodbakery'),
			    'desc' => '',
			    'hint_text' => '',
			    'echo' => true,
			    'field_params' => array(
				...
				'options' => array('order-deatail' => __('Order Detail', 'foodbakery'), 'restaurant' => __('Add Restaurant', 'foodbakery')),
				'return' => true,
			    ),
			);
	    	...
	    	<input id="dir-add-column-<?php echo absint($rand_numb) ?>" data-id="<?php echo absint($rand_numb) ?>" class="dir-add-column" type="button" value="<?php _e('Add Membership', 'foodbakery') ?>">
	    	<input id="dir-add-row-<?php echo absint($rand_numb) ?>" data-id="<?php echo absint($rand_numb) ?>" class="dir-add-row" type="button" value="<?php _e('Add Row', 'foodbakery') ?>">
	    	<input id="dir-add-section-<?php echo absint($rand_numb) ?>" data-id="<?php echo absint($rand_numb) ?>" class="dir-add-section" type="button" value="<?php _e('Add Section', 'foodbakery') ?>">
	    	<input id="dir-cols-reset-<?php echo absint($rand_numb) ?>" data-id="<?php echo absint($rand_numb) ?>" class="dir-cols-reset" type="button" value="<?php _e('Reset All', 'foodbakery') ?>">
	        </div>
	        <table id="foodbakery-price-table-<?php echo absint($rand_numb) ?>" data-id="<?php echo absint($rand_numb) ?>" cellpadding="5" cellspacing="0" border="1">
	    	<tbody>
			<?php
			$table_html = '';
			...
			$pt_pkg_name = get_post_meta($post->ID, 'foodbakery_pt_pkg_names', true);
			$pt_pkg_price = get_post_meta($post->ID, 'foodbakery_pt_pkg_prices', true);
			$pt_pkg_color = get_post_meta($post->ID, 'foodbakery_pt_pkg_color', true);
			$pt_pkg_desc = get_post_meta($post->ID, 'foodbakery_pt_pkg_descs', true);
			$pt_pkg_btn_txt = get_post_meta($post->ID, 'foodbakery_pt_pkg_btn_txts', true);
			$pt_pkg_dur = get_post_meta($post->ID, 'foodbakery_pt_pkg_durs', true);
			$pt_pkg_feat = get_post_meta($post->ID, 'foodbakery_pt_pkg_feats', true);
			$pt_pkg_url = get_post_meta($post->ID, 'foodbakery_pt_pkg_urls', true);
			...
			if (
				...
				$def_pkg_price_marup .= '<td class="pkg-price"><input  type="text" name="pt_pkg_price[]" value="' . $def_pkg_price . '"></td>' . "\n";
				...
				$def_pkg_feat_marup .= '<td class="pkg-featured"><select name="pt_pkg_feat[]"><option value="no">' . __('No', 'foodbakery') . '</option><option' . ($def_pkg_feat == 'yes' ? ' selected="selected"' : '') . ' value="yes">' . __('Yes', 'foodbakery') . '</option></select></td>' . "\n";
				$def_pkg_url_marup .= '<td class="pkg-url">' . $this->foodbakery_pkgs($def_pkg_url) . '</td>' . "\n";
				//
				$pkgs_counter ++;
			    }
			}
			...
			if ($def_pkg_name_marup == '' && $def_pkg_price_marup == '' && $def_pkg_desc_marup == '') {
			    ...
			    for ($pri = 0; $pri < 3; $pri ++) {
				$def_pkg_name_marup .= '<td class="pkg-name"><input type="text" name="pt_pkg_name[]" value="Membership ' . (1 + $pri) . '"></td>' . "\n";
				$def_pkg_price_marup .= '<td class="pkg-price"><input type="text" name="pt_pkg_price[]" value="$20"></td>' . "\n";
				...
				$def_pkg_btn_txt_marup .= '<td class="pkg-btn-text"><input type="text" name="pt_pkg_btn_txt[]" value="Buy Now"></td>' . "\n";
				$def_pkg_dur_marup .= '<td class="pkg-duration"><input type="text" name="pt_pkg_dur[]" value="' . (7 + $pri) . '"></td>' . "\n";
				$def_pkg_feat_marup .= '<td class="pkg-featured"><select name="pt_pkg_feat[]"><option value="no">' . __('No', 'foodbakery') . '</option><option value="yes">' . __('Yes', 'foodbakery') . '</option></select></td>' . "\n";
				$def_pkg_url_marup .= '<td class="pkg-url">' . $this->foodbakery_pkgs('') . '</td>' . "\n";
			    }
			}

			$default_field = '
						<tr class="default-fields">
							<td class="pkg-name">' . __('Name', 'foodbakery') . '</td>
							' . $def_pkg_name_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-price">' . __('Price', 'foodbakery') . '</td>
							' . $def_pkg_price_marup . '
						</tr>
                                                                                                            <tr class="default-fields">
							<td class="pkg-color">' . __('Color', 'foodbakery') . '</td>
							' . $def_pkg_color_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-desc">' . __('Description', 'foodbakery') . '</td>
							' . $def_pkg_desc_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-btn-text">' . __('Button Text', 'foodbakery') . '</td>
							' . $def_pkg_btn_txt_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-duration">' . __('Duration', 'foodbakery') . '</td>
							' . $def_pkg_dur_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-featured">' . __('Featured', 'foodbakery') . '</td>
							' . $def_pkg_feat_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-url">' . __('Membership', 'foodbakery') . '</td>
							' . $def_pkg_url_marup . '
						</tr>';
			...
			$row_num_input = get_post_meta($post->ID, 'foodbakery_pt_row_num', true);
			$pt_col_input = get_post_meta($post->ID, 'foodbakery_pt_col_vals', true);
			$pt_col_sub_input = get_post_meta($post->ID, 'foodbakery_pt_col_subs', true);
			$pt_row_title = get_post_meta($post->ID, 'foodbakery_pt_row_title', true);
			$pt_sec_val = get_post_meta($post->ID, 'foodbakery_pt_sec_vals', true);
			$pt_sec_pos = get_post_meta($post->ID, 'foodbakery_pt_sec_pos', true);
			if (
				$row_num_input &&
				...
				foreach ($pt_col_input as $col_val) {
				    ...
				    $col_markup .= '<td><input type="text" name="dir_pt_col_val[]" size="10" value="' . $col_val . '"> <br> ' . $this->foodbakery_pt_icon($pt_sub_input_val, $rand_numb, 'dir_pt_col_sub') . (1 == $pt_counter ? $row_num_field : '') . '</td>' . "\n";
				    $pt_row_del = '';
				...
				for ($pri = 0; $pri < 6; $pri ++) {
				    $rand_numb = rand(10000000, 99999999);
				    $col_markup .= '<td><input type="text" name="dir_pt_col_val[]" size="10" value="Title"> <br> ' . $this->foodbakery_pt_icon('', $rand_numb, 'dir_pt_col_sub') . (1 == $pt_counter ? $row_num_field : '') . '</td>' . "\n";
				    $pt_row_del = '';
				    if ($row_break == $pt_counter) {
	    ...
	    <?php
	    $this->submit_meta_box('foodbakery-pt', $args = array());
	}
	...
	public function foodbakery_pkgs($value = '') {
	    ...
	    $pkgs_options .= '<option value="">' . __('Select Membership', 'foodbakery') . '</option>';
	...
	public function foodbakery_insert_price_table_metas($post_id) {

	    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
		return;
	    }

	    $row_num_input = foodbakery_get_input('dir_pt_row_num', '', 'ARRAY');
	    $pt_col_input = foodbakery_get_input('dir_pt_col_val', '', 'ARRAY');
	    $pt_col_sub_input = foodbakery_get_input('dir_pt_col_sub', '', 'ARRAY');
	    $pt_row_title = foodbakery_get_input('dir_pt_row_title', '', 'ARRAY');
	    $pt_sec_val = foodbakery_get_input('dir_pt_sec_val', '', 'ARRAY');
	    $pt_sec_pos = foodbakery_get_input('dir_pt_sec_pos', '', 'ARRAY');

	    $pt_pkg_name = foodbakery_get_input('pt_pkg_name', '', 'ARRAY');
	    $pt_pkg_price = foodbakery_get_input('pt_pkg_price', '', 'ARRAY');
	    $pt_pkg_color = foodbakery_get_input('pt_pkg_color', '', 'ARRAY');
	    $pt_pkg_desc = foodbakery_get_input('pt_pkg_desc', '', 'ARRAY');
	    $pt_pkg_btn_txt = foodbakery_get_input('pt_pkg_btn_txt', '', 'ARRAY');
	    $pt_pkg_dur = foodbakery_get_input('pt_pkg_dur', '', 'ARRAY');
	    $pt_pkg_feat = foodbakery_get_input('pt_pkg_feat', '', 'ARRAY');
	    $pt_pkg_url = foodbakery_get_input('pt_pkg_url', '', 'ARRAY');

	    // saving package names		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_names', $pt_pkg_name);

	    // saving package prices		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_prices', $pt_pkg_price);
	    update_post_meta($post_id, 'foodbakery_pt_pkg_color', $pt_pkg_color);

	    // saving package descs		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_descs', $pt_pkg_desc);

	    // saving package btn texts		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_btn_txts', $pt_pkg_btn_txt);

	    // saving package durs		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_durs', $pt_pkg_dur);

	    // saving package feats		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_feats', $pt_pkg_feat);

	    // saving package urls		
	    update_post_meta($post_id, 'foodbakery_pt_pkg_urls', $pt_pkg_url);

	    // saving num of rows	
	    if (!empty($row_num_input) && is_array($row_num_input) && count($row_num_input) > 0) {
		
		update_post_meta($post_id, 'foodbakery_pt_row_num', absint(count($row_num_input)));
		
	    }

	    // saving all cols values			
	    update_post_meta($post_id, 'foodbakery_pt_col_vals', $pt_col_input);

	    // saving all cols sub values			
	    update_post_meta($post_id, 'foodbakery_pt_col_subs', $pt_col_sub_input);

	    // saving row titles values			
	    update_post_meta($post_id, 'foodbakery_pt_row_title', $pt_row_title);

	    // saving all section values
	    update_post_meta($post_id, 'foodbakery_pt_sec_vals', $pt_sec_val);

	    // saving all section pos values
	    update_post_meta($post_id, 'foodbakery_pt_sec_pos', $pt_sec_pos);
	}

	public function foodbakery_pt_icon($value = '', $id = '', $name = '') {//begin function
	    if ($value == '' && $id == '' && $name == '') {
