<?php
/**
 * Membership Purchased Email Template.
 *
 * @since 1.0
 * @package	Foodbakery
 */

if ( ! class_exists( 'Foodbakery_package_purchased_email_template' ) ) {

	class Foodbakery_package_purchased_email_template {
		...
		public function __construct() {
			...
			add_action( 'foodbakery_package_purchased_email', array( $this, 'foodbakery_package_purchased_email_callback' ), 10, 2 );
		}

		public function foodbakery_package_purchased_email_callback( $user = '', $package_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : esc_html__( 'Membership Purchased', 'foodbakery' );
				...
				Foodbakery_package_purchased_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => esc_html__( 'This template is used to send email when new package purchased.', 'foodbakery' ),
				'jh_email_type' => 'html',
			);
	...
	new Foodbakery_package_purchased_email_template();
}
