...
if ( ! class_exists( 'Foodbakery_restaurant_add_email_template' ) ) {

	class Foodbakery_restaurant_add_email_template {
		...
		public function __construct() {
			...
			add_action( 'foodbakery_restaurant_add_email', array( $this, 'foodbakery_restaurant_add_email_callback' ), 10, 2 );
		}
		...
		public function foodbakery_restaurant_add_email_callback( $user = '', $restaurant_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_restaurant_added_email();
				...
				Foodbakery_restaurant_add_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
	...
	new Foodbakery_restaurant_add_email_template();
}
