...
function($) {
	...
	// ======================
	var Aromodal = function(element, options) {
	...
	Aromodal.prototype.toggle = function(_relatedTarget) {
		return this.isShown ? this.hide() : this.show(_relatedTarget)
	}
	Aromodal.prototype.show = function(_relatedTarget) {
		var that = this
		...
		this.backdrop(function() {
			var transition = $.support.transition && that.$element.hasClass('fade-aro')
			...
			transition ?
				...
				.one('bsTransitionEndAro', function() {
					that.$element.trigger('focus.aro').trigger(e)
				})
				.emulateTransitionEndAro(Aromodal.TRANSITION_DURATION) :
				that.$element.trigger('focus.aro').trigger(e)
		})
	}
	Aromodal.prototype.hide = function(e) {
		...
		$.support.transition && this.$element.hasClass('fade-aro') ?
			this.$element
			.one('bsTransitionEndAro', $.proxy(this.hideAromodal, this))
			.emulateTransitionEndAro(Aromodal.TRANSITION_DURATION) :
			this.hideAromodal()
	}
	Aromodal.prototype.enforceFocus = function() {
		$(document)
			...
			.on('focusin.bs.aromodal', $.proxy(function(e) {
				if (document !== e.target &&
					...
					this.$element.trigger('focus.aro')
				}
			}, this))
	}
	Aromodal.prototype.escape = function() {
	...
	Aromodal.prototype.resize = function() {
		if (this.isShown) {
	...
	Aromodal.prototype.hideAromodal = function() {
		var that = this
		this.$element.hide()
		this.backdrop(function() {
	...
	Aromodal.prototype.removeBackdrop = function() {
		this.$backdrop && this.$backdrop.remove()
		this.$backdrop = null
	}
	Aromodal.prototype.backdrop = function(callback) {
		var that = this
		...
		if (this.isShown && this.options.backdrop) {
			...
			doAnimate ?
				this.$backdrop
				.one('bsTransitionEndAro', callback)
				.emulateTransitionEndAro(Aromodal.BACKDROP_TRANSITION_DURATION) :
				callback()
		} else if (!this.isShown && this.$backdrop) {
			this.$backdrop.removeClass('in-aro')
			...
			$.support.transition && this.$element.hasClass('fade-aro') ?
				this.$backdrop
				.one('bsTransitionEndAro', callbackRemove)
				.emulateTransitionEndAro(Aromodal.BACKDROP_TRANSITION_DURATION) :
				callbackRemove()
		} else if (callback) {
			callback()
	...
	Aromodal.prototype.handleUpdate = function() {
		this.adjustDialog()
	}
	Aromodal.prototype.adjustDialog = function() {
	...
	Aromodal.prototype.resetAdjustments = function() {
		this.$element.css({
			paddingLeft: '',
			paddingRight: ''
		})
	}
	Aromodal.prototype.checkScrollbar = function() {
	...
	Aromodal.prototype.setScrollbar = function() {
		var bodyPad = parseInt((this.$body.css('padding-right') || 0), 10)
		this.originalBodyPad = document.body.style.paddingRight || ''
		if (this.bodyIsOverflowing) this.$body.css('padding-right', bodyPad + this.scrollbarWidth)
	}
	Aromodal.prototype.resetScrollbar = function() {
		this.$body.css('padding-right', this.originalBodyPad)
	}
	Aromodal.prototype.measureScrollbar = function() { // thx walsh
	...
	$(document).on('click.bs.aromodal.data-api', '[data-toggle="aromodal"]', function(e) {
		...
		$target.one('show.bs.aromodal', function(showEvent) {
			...
			$target.one('hidden.bs.aromodal', function() {
				$this.is(':visible') && $this.trigger('focus.aro')
			})
		})
...
function($) {
	...
	Tooltip.prototype.enter = function(obj) {
		...
		if (self.tip().hasClass('in-aro') || self.hoverState == 'in') {
			self.hoverState = 'in'
			return
		}
	...
	Tooltip.prototype.show = function() {
		...
		if (this.hasContent() && this.enabled) {
			...
			$.support.transition && this.$tip.hasClass('fade-aro') ?
				$tip
				.one('bsTransitionEndAro', complete)
				.emulateTransitionEndAro(Tooltip.TRANSITION_DURATION) :
				complete()
		}
	}
	...
	Tooltip.prototype.hide = function(callback) {
		...
		$.support.transition && $tip.hasClass('fade-aro') ?
			$tip
			.one('bsTransitionEndAro', complete)
			.emulateTransitionEndAro(Tooltip.TRANSITION_DURATION) :
			complete()
	...
	Tooltip.prototype.toggle = function(e) {
		...
		if (e) {
			...
			self.tip().hasClass('in-aro') ? self.leave(self) : self.enter(self)
		}
	}
...
+function ($) {
  ...
  $.fn.emulateTransitionEndAro = function (duration) {
    var called = false
    var $el = this
  ...
  $(function () {
    ...
    $.event.special.bsTransitionEndAro = {
...
/* ========================================================================
 * Bootstrap: arocollapse.js v3.4.1
 * https://getbootstrap.com/docs/3.4/javascript/#arocollapse
 * ========================================================================
...
+function ($) {
  ...
  var Collapse = function (element, options) {
    ...
    this.$trigger      = $('[data-toggle="arocollapse"][href="#' + element.id + '"],' +
                           '[data-toggle="arocollapse"][data-target="#' + element.id + '"]')
  ...
  Collapse.prototype.show = function () {
    if (this.transitioning || this.$element.hasClass('in-aro')) return

    var activesData
    var actives = this.$parent && this.$parent./*children('.fdoe').*/children('.in-aro, .arocollapsing')
    ...
    this.$element
      .removeClass('arocollapse')
      .addClass('arocollapsing')[dimension](0)
      .attr('aria-expanded', true)

    this.$trigger
      .removeClass('arocollapsed')
      .attr('aria-expanded', true)
    ...
    var complete = function () {
      this.$element
        .removeClass('arocollapsing')
        .addClass('arocollapse in-aro')[dimension]('')
   ...
   var scrollSize = $.camelCase(['scroll', dimension].join('-'))
    ...
    this.$element
      .one('bsTransitionEndAro', $.proxy(complete, this))
      .emulateTransitionEndAro(Collapse.TRANSITION_DURATION)[dimension](this.$element[0][scrollSize])
  }
  ...
  Collapse.prototype.hide = function () {
    ...
    this.$element
      .addClass('arocollapsing')
      .removeClass('arocollapse in-aro')
      .attr('aria-expanded', false)

    this.$trigger
      .addClass('arocollapsed')
      .attr('aria-expanded', false)
    ...
    this.$element
      [dimension](0)
      .one('bsTransitionEndAro', $.proxy(complete, this))
      .emulateTransitionEndAro(Collapse.TRANSITION_DURATION)
  }

  Collapse.prototype.toggle = function () {
    this[this.$element.hasClass('in-aro') ? 'hide' : 'show']()
  }

  Collapse.prototype.getParent = function () {
    return $(document).find(this.options.parent)
      .find('[data-toggle="arocollapse"][data-parent="' + this.options.parent + '"]')
  ...
  Collapse.prototype.addAriaAndCollapsedClass = function ($element, $trigger) {
    var isOpen = $element.hasClass('in-aro')

    $element.attr('aria-expanded', isOpen)
    $trigger
      .toggleClass('arocollapsed', !isOpen)
...
var old = $.fn.arocollapse
  ...
  $(document).on('click.bs.arocollapse.data-api', '[data-toggle="arocollapse"]', function (e) {
    var $this   = $(this)
